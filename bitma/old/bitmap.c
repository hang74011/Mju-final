...
int main (int argc, char **argv)
{
    ...
    if( ioctl(fbfd, FBIOGET_VSCREENINFO, &fbvar) )//frame buffer 퉁침.
    {
    ...
    if (fbvar.bits_per_pixel != 32) //32bit 사용.
    {
    ...
    pfbmap  =   (unsigned char *)
        mmap(0, mem_size*4, PROT_READ|PROT_WRITE, MAP_SHARED, fbfd, 0);//유저 메모리영역에 커널 메모리영역할당 요청.,곱하기 4필요(4byte라?)
    ...
    // fb clear - black 스크린 헤이트가 600 위스가 1024 
